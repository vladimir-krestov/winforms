    ...
    public partial class MonthCalendar
    {
        internal class MonthCalendarAccessibleObject : ControlAccessibleObject
        {
            ...
            public MonthCalendarAccessibleObject(MonthCalendar owner) : base(owner)
            {
                ...
                _owningMonthCalendar.GotFocus += OnMonthCalendarGotFocus;
            }
            ...
            // One more reason is to be consistent with the rest collections of calendar parts accessible objects.
            ...
            internal override UiaCore.IRawElementProviderFragment? ElementProviderFromPoint(double x, double y)
            {
                ...
                return hitTestInfo.uHit switch
                {
                    ...
                    MCHT.CALENDARDATEMAX // The given point was over the maximum date in the calendar
                        ...
                        : this,
                    MCHT.CALENDAR
                        => CalendarsAccessibleObjects.FirstOrDefault(c => c.Bounds.Contains(innerX, innerY))
                    ...
                    MCHT.TITLE or MCHT.TITLEMONTH or MCHT.TITLEYEAR
                        => CalendarsAccessibleObjects.FirstOrDefault(c => c.CalendarHeaderAccessibleObject
                        .Bounds.Contains(innerX, innerY))?.CalendarHeaderAccessibleObject
                        // Cast "this" to AccessibleObject because "??" can't cast these operands implicitly
                        ?? (AccessibleObject)this,
                    ...
                    or MCHT.CALENDARDATEPREV or MCHT.CALENDARDATENEXT
                        ...
                        // Cast "this" to AccessibleObject because "??" can't cast these operands implicitly
                        => CalendarsAccessibleObjects.FirstOrDefault(c => c.Bounds.Contains(innerX, innerY))
                        ?.GetChildFromPoint(hitTestInfo) ?? (AccessibleObject)this,
                    MCHT.NOWHERE => this,
            ...
            internal unsafe string GetCalendarPartText(MCGIP dwPart, int calendarIndex = 0, int rowIndex = 0, int columnIndex = 0)
            {
                ...
                return name.SliceAtFirstNull().ToString();
            }
            ...
            private CalendarCellAccessibleObject? GetCellByDate(DateTime date)
            {
                foreach (CalendarAccessibleObject calendar in CalendarsAccessibleObjects)
                {
                    ...
                    {
                        foreach (CalendarRowAccessibleObject row in calendar.CalendarBodyAccessibleObject.RowsAccessibleObjects)
                        {
            ...
            internal SelectionRange GetDisplayRange(bool visible) => _owningMonthCalendar.GetDisplayRange(visible);
            ...
            internal override UiaCore.IRawElementProviderSimple? GetItem(int row, int column)
                => CalendarsAccessibleObjects.FirstOrDefault(c => c.Row == row && c.Column == column);
            ...
            private void OnMonthCalendarGotFocus(object? sender, EventArgs e)
            {
                if (_calendarsAccessibleObjects is null)
                {
                    // Don't raise the UIA event for the focused cell
                    // if the calendars collection is not created
                    // because there are no any accessibility listeners
                    // that should take this accessible object children
                    // thereby build the calendars collection.
                    return;
                }
            ...
            private void OnMonthCalendarStateChanged(object? sender, EventArgs e) => RebuildAccessibilityTree();
            ...
            internal void SetSelectionRange(DateTime d1, DateTime d2) => _owningMonthCalendar.SetSelectionRange(d1, d2);
            ...
            public override string? Value
            {
                get
                {
                    ...
                    switch (_owningMonthCalendar._mcCurView)
                    {
                        case MCMV.MONTH:
